...
const eventUpdateRoute = new Hono<{
  Variables: MiddlewareData;
}>();

eventUpdateRoute.put(
  "/details",
  zValidator("json", eventDetailsSchema),
  async (c) => {
...
eventUpdateRoute.delete("/coupon", zValidator("json", idSchema), async (c) => {
  const body = c.req.valid("json");
  const userId = c.get("userId");
